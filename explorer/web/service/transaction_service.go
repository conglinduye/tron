...
func QueryTransactionsByAddress(req *entity.Transactions) (*entity.TransactionsResp, error) {
	...
	mutiFilter := false
	strSQL := fmt.Sprintf(`
	select oo.block_id,oo.own_address,oo.to_address,oo.contract_type,oo.trx_hash,oo.create_time from (
	SELECT block_id,own_address,to_address,contract_type,trx_hash,create_time 
